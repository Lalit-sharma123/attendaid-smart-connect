    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => toast.success("Excel export queued")} className="inline-flex items-center gap-2 h-9 px-3 rounded-md border text-sm"><FileSpreadsheet className="size-4" /> Excel</button>
          <button onClick={() => toast.success("PDF export queued")} className="inline-flex items-center gap-2 h-9 px-3 rounded-md border text-sm"><FileText className="size-4" /> PDF</button>
          <button onClick={() => downloadCSV(`report-${Date.now()}.csv`, attendanceRecords)} className="inline-flex items-center gap-2 h-9 px-3 rounded-md border text-sm"><Download className="size-4" /> CSV</button>
          <button onClick={() => window.print()} className="inline-flex items-center gap-2 h-9 px-3 rounded-md border text-sm"><Printer className="size-4" /> Print</button>
          <button onClick={() => setScheduleOpen(true)} className="inline-flex items-center gap-2 h-9 px-3 rounded-md bg-primary text-primary-foreground text-sm"><Mail className="size-4" /> Schedule Email</button>
        </div>
      </div>
          ...
          return (
            <div key={r.title} className="rounded-xl border bg-card p-5 hover:shadow-md transition-shadow">
              ...
              <div className="flex gap-2">
                <button onClick={() => setPreview(r.title)} className="flex-1 h-8 rounded-md bg-secondary text-xs hover:bg-accent">Preview</button>
                <button onClick={() => { downloadCSV(`${r.title.replace(/\s+/g, "-").toLowerCase()}.csv`, attendanceRecords); toast.success("Generated"); }} className="flex-1 h-8 rounded-md bg-primary text-primary-foreground text-xs">Generate</button>
              </div>
            </div>
          );
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6">
        ...
        <SectionCard title="Export Options">
          <div className="space-y-3 text-sm">
            ...
            <button onClick={() => { downloadCSV("custom-export.csv", attendanceRecords); toast.success("Export complete"); }} className="w-full h-10 rounded-md bg-primary text-primary-foreground text-sm font-medium">Export Report</button>
          </div>
        </SectionCard>
      </div>
      ...
      <Dialog open={!!preview} onOpenChange={(o) => !o && setPreview(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader><DialogTitle>{preview}</DialogTitle><DialogDescription>Live preview · sample 8 rows</DialogDescription></DialogHeader>
          <div className="rounded-lg border bg-secondary/20 p-4 max-h-96 overflow-y-auto text-xs">
            <table className="w-full"><thead><tr className="text-left text-muted-foreground"><th>Roll</th><th>Name</th><th>Status</th></tr></thead>
              <tbody>{attendanceRecords.map((r) => <tr key={r.roll + r.time} className="border-t"><td className="py-1 font-mono">{r.roll}</td><td>{r.name}</td><td>{r.status}</td></tr>)}</tbody></table>
          </div>
          <DialogFooter><button onClick={() => { downloadCSV(`${preview}.csv`, attendanceRecords); toast.success("Downloaded"); setPreview(null); }} className="h-9 px-4 rounded-md bg-primary text-primary-foreground text-sm">Download CSV</button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={scheduleOpen} onOpenChange={setScheduleOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Schedule Email Report</DialogTitle><DialogDescription>Receive reports automatically.</DialogDescription></DialogHeader>
          <div className="space-y-3 text-sm">
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="recipient@example.com" className="w-full h-9 px-3 rounded-md border bg-card" />
            <select value={freq} onChange={(e) => setFreq(e.target.value)} className="w-full h-9 px-3 rounded-md border bg-card">
          ...
          <DialogFooter>
            <button onClick={() => { if (!email) { toast.error("Email required"); return; } toast.success(`Scheduled · ${freq}`); setScheduleOpen(false); }} className="h-9 px-4 rounded-md bg-primary text-primary-foreground text-sm">Schedule</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
